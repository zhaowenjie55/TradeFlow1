...
export interface ReportTrustIndicator {
...
function buildModifierPrefix(title: string) {
...
function simplifyChineseHint(value?: string | null) {
  if (!value) return ""
...
function simplifyEnglishTitle(title: string) {
  const tokens = title
    .replace(/[,_/()-]+/g, " ")
    .split(/\s+/)
    .map((item) => item.trim())
...
export function getReadableProductName(
  title?: string | null,
  options?: { domesticHint?: string | null },
) {
  if (!title) return "商品"
...
export function getReadableOriginalTitle(title?: string | null, readableName?: string | null) {
  if (!title) return ""
  if (!readableName || readableName === title) return ""
  return title.length > 96 ? `${title.slice(0, 96)}...` : title
}

export function getReadableMatchTitle(match: DomesticProductMatch) {
  return getReadableProductName(match.title, { domesticHint: match.title })
}

export function getReportFileName(title?: string | null, domesticHint?: string | null) {
  const base = getReadableProductName(title, { domesticHint })
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, "")
    .trim()
  return `${base || "商品"}-套利报告.pdf`
}

export function compactNarrativeSummary(
  message?: string | null,
  options?: { productTitle?: string | null; domesticTitle?: string | null },
) {
  if (!message) return ""
  ...
  if (options?.productTitle) normalized = normalized.replaceAll(options.productTitle, productName)
  if (options?.domesticTitle) normalized = normalized.replaceAll(options.domesticTitle, domesticName)
...
export function humanizeTaskLog(entry: TaskLogEntry) {
  const raw = entry.message.trim()
  let message = raw
  if (/Phase1 任务已创建并进入队列/.test(raw)) message = "任务已经建好，先去拉一轮海外候选数据。"
  else if (/开始执行商品库驱动的 Amazon 候选检索/.test(raw)) message = "先从海外样本里扫一遍相关商品，把能做候选的结果抓出来。"
  else if (/Amazon 商品库未命中/.test(raw)) message = "实时结果不够，我先切到历史快照补齐样本，避免候选直接断掉。"
  else if (/已按关键词相似度/.test(raw)) message = "候选已经按相关度、排序位置、口碑和价格带重新压了一遍。"
  else if (/候选商品已生成/.test(raw)) message = "第一批候选已经整理完，可以继续挑一个做深度分析。"
  else if (/Phase2 任务已创建/.test(raw)) message = "接管你选中的商品，开始往国内货源、成本和利润链路继续深挖。"
  else if (/开始执行国内货源匹配与 ROI 试算/.test(raw)) message = "先把国内供货、运费和 ROI 框架同时跑起来，看看这单值不值得追。"
  else if (/已完成真实 GLM 改写/.test(raw)) message = "先压缩出标题里的核心卖点，再生成一组更贴近国内搜索习惯的检索词。"
  else if (/已完成商品库混合检索与语义匹配/.test(raw)) message = "关键词检索和语义召回已经一起跑完，正在保留更像同款的候选。"
  else if (/1688/.test(raw) && /验证/.test(raw)) message = "检测到 1688 风控，请先在浏览器窗口完成登录或滑块验证，再点击继续抓取。"
  else if (/已加载商品详情快照/.test(raw)) message = "详情快照已经补齐，品牌、规格和属性可以一起纳入判断。"
  else if (/未命中商品详情快照/.test(raw)) message = "详情没有完全命中，我先用标题、价格和检索证据继续往下推。"
  else if (/已完成成本公式测算与利润估算/.test(raw)) message = "成本、汇率、平台抽佣和物流都过了一遍，利润空间已经基本收口。"
  else if (/结构化报告已生成/.test(raw)) message = "结论已经整理完成，右侧报告现在可以直接看。"
...
export function getVerificationGuidance(logs: TaskLogEntry[], t: Translate): VerificationGuidance {
  const relevantEntry = [...logs]
    .reverse()
    .find(
      (entry) =>
        entry.stage === "phase2.verification" ||
        /1688|验证|captcha|风控|slider|login/i.test(entry.message),
    )

  return {
    title: t("workspace.verificationBannerTitle"),
    detail: relevantEntry ? humanizeTaskLog(relevantEntry).message : t("workspace.voiceRiskTitle"),
    hint: t("workspace.verificationBannerHint"),
  }
}

export function resolveTaskLogCategory(entry: TaskLogEntry) {
...
export function isKeyTaskLog(entry: TaskLogEntry) {
  return resolveTaskLogCategory(entry) !== "system" || entry.level === "ERROR"
}

export function getLiveStageMessage(stage?: string | null) {
  if (!stage) return "正在继续补充分析过程"
  return LIVE_STAGE_MESSAGES[stage] ?? "正在继续补充分析过程"
}

export function getReadableStageLabel(stage?: string | null) {
  if (!stage) return "待命"
  return LOG_STAGE_LABELS[stage] ?? stage
}

export function getReadableLlmProvider(provider?: string | null) {
  if (!provider) return "--"
  const labels: Record<string, string> = {
    GLM_CHAT: "实时 LLM",
    SIMULATED_LLM: "模拟 LLM 回退",
  }
  return labels[provider] ?? provider
}

export function humanizeReportSource(
  value?: string | null,
  t?: (key: string) => string,
) {
  if (!value) return "--"
...
export function humanizeQualityTier(
  value?: string | null,
  t?: (key: string) => string,
) {
  if (!value) return "--"
...
export function getReportTrustIndicator(
  qualityTier: string | null | undefined,
  fallbackUsed: boolean,
  t: Translate,
): ReportTrustIndicator {
  switch (qualityTier) {
    case "REALTIME_CONFIRMED":
      return {
        tone: "success",
        title: t("report.trustConfirmedTitle"),
        description: t("report.trustConfirmedDescription"),
      }
    case "REALTIME_HYBRID":
      return {
        tone: "info",
        title: t("report.trustHybridTitle"),
        description: t("report.trustHybridDescription"),
      }
    case "SNAPSHOT_FALLBACK":
      return {
        tone: "warning",
        title: t("report.trustSnapshotTitle"),
        description: t("report.trustSnapshotDescription"),
      }
    case "LLM_FALLBACK_ASSISTED":
      return {
        tone: "danger",
        title: t("report.trustFallbackTitle"),
        description: t("report.trustFallbackDescription"),
      }
    default:
      if (fallbackUsed) {
        return {
      ...
      return {
        tone: "info",
        title: t("report.trustReviewTitle"),
        description: t("report.trustReviewDescription"),
      }
  }
}
